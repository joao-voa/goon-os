...
const NEON = '#C7F900', INK = '#0f172a', MUT = '#64748b', DIM = '#94a3b8', LINE = '#e2e8f0', BG = '#f8fafc', CARD = '#fff', SLATE = '#475569'
const GREEN = '#16a34a', RED = '#dc2626', AMBER = '#f59e0b'
...
export function ClientMentorship({ clientId }: { clientId: string }) {
  ...
  if (enrolled === null) return <div style={{ fontFamily: mono, fontSize: 13, color: MUT }}>Carregando...</div>
  if (!enrolled) {
    return (
      <div style={{ border: `1px solid ${LINE}`, borderRadius: 12, padding: 24, textAlign: 'center', background: CARD, boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
        <div style={{ fontFamily: mono, fontSize: 14, color: INK, marginBottom: 12 }}>Este cliente não está em acompanhamento de mentoria.</div>
        <button onClick={enroll} style={{ background: NEON, color: INK, border: 0, borderRadius: 6, padding: '9px 16px', fontFamily: mono, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ Inscrever no acompanhamento</button>
      </div>
    )
  }
  if (!d) return null
  ...
  const h: React.CSSProperties = { fontFamily: mono, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: MUT, marginBottom: 10 }
  const card: React.CSSProperties = { border: `1px solid ${LINE}`, borderRadius: 12, background: CARD, boxShadow: '0 1px 2px rgba(0,0,0,0.04)', padding: 16, marginBottom: 14 }
  const allChannels = [...new Set(d.caseStudies.flatMap(cs => (cs.vendasPorCanal ?? []).map(c => c.canal)))]
  const th: React.CSSProperties = { padding: '8px 10px', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600 }

  return (
    <div>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ fontFamily: mono, fontSize: 13, color: MUT }}>
          Mentor: <b style={{ color: INK }}>{d.profile.mentorName ?? '—'}</b> · {d.profile.status} · <b style={{ color: INK }}>{doneCount}</b> sessões
          {d.attention && <span style={{ marginLeft: 8, background: '#fee2e2', color: RED, padding: '2px 10px', borderRadius: 100, fontWeight: 700, fontSize: 11 }}>Em atenção</span>}
        </div>
        <button onClick={() => setShowForm(true)} style={{ background: NEON, color: INK, border: 0, borderRadius: 6, padding: '9px 16px', fontFamily: mono, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ Registrar sessão</button>
      </div>

      {/* Contexto */}
      <div style={card}>
        <div style={h}>Contexto</div>
        <div style={{ fontFamily: mono, fontSize: 13, color: INK, marginBottom: 6 }}><b>Dores:</b> {d.profile.mainPains || <span style={{ color: DIM }}>—</span>}</div>
        <div style={{ fontFamily: mono, fontSize: 13, color: INK }}><b>Objetivo:</b> {d.profile.goal || <span style={{ color: DIM }}>—</span>}</div>
      </div>
      ...
      {tab === 'evolucao' && (
        <div style={{ ...card, overflowX: 'auto' }}>
          ...
          {d.caseStudies.length === 0 ? <div style={{ fontFamily: mono, fontSize: 13, color: MUT }}>Nenhuma sessão registrada.</div> : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: mono, fontSize: 12 }}>
              <thead>
                <tr style={{ background: BG, color: MUT }}>
                  ...
                  <th style={{ ...th, textAlign: 'right' }}>IG</th>
                  {allChannels.map(c => <th key={c} style={{ ...th, textAlign: 'right' }}>{c}</th>)}
                </tr>
              </thead>
              <tbody>
                {d.caseStudies.slice().reverse().map(cs => (
                  <tr key={cs.id} style={{ borderTop: `1px solid ${LINE}` }}>
                    <td style={{ padding: '8px 10px', fontWeight: 700, color: INK }}>{dt(cs.sessionDate)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{brl(cs.faturamentoAno)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{brl(cs.ticketMedio)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{num(cs.numClientes)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{num(cs.numVendas)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{cs.roas != null ? cs.roas + 'x' : '—'}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{num(cs.seguidoresIg)}</td>
                    {allChannels.map(c => <td key={c} style={{ padding: '8px 10px', textAlign: 'right', ...tnum }}>{brl((cs.vendasPorCanal ?? []).find(x => x.canal === c)?.valor ?? null)}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {last?.customFields && last.customFields.length > 0 && (
            <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {last.customFields.map((f, i) => <div key={i} style={{ border: `1px solid ${LINE}`, borderRadius: 6, padding: '5px 10px', fontFamily: mono, fontSize: 11, color: INK }}><span style={{ color: MUT }}>{f.label}:</span> <b>{f.value}</b></div>)}
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'sessoes' && (
        <div>
          {d.caseStudies.length === 0 && <div style={{ ...card, fontFamily: mono, fontSize: 13, color: MUT }}>Nenhuma sessão registrada.</div>}
          {d.caseStudies.map(cs => (
            <div key={cs.id} style={card}>
              <div onClick={() => setExpanded(expanded === cs.id ? null : cs.id)} style={{ display: 'flex', justifyContent: 'space-between', cursor: 'pointer', fontFamily: mono, fontSize: 13, fontWeight: 700, color: INK }}>
                <span>{expanded === cs.id ? '▾' : '▸'} Sessão {dt(cs.sessionDate)}{cs.mentorName ? ` · ${cs.mentorName}` : ''}</span>
                <span style={{ color: MUT, fontWeight: 500, ...tnum }}>{brl(cs.faturamentoAno)}</span>
              </div>
              {expanded === cs.id && (
                <div style={{ marginTop: 12, fontFamily: mono, fontSize: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {cs.oQueTrabalhou && <div><b style={{ color: INK }}>O que foi passado:</b> <span style={{ color: SLATE }}>{cs.oQueTrabalhou}</span></div>}
                  ...
                  {cs.transcricao && <details><summary style={{ cursor: 'pointer', color: MUT }}>Transcrição</summary><div style={{ whiteSpace: 'pre-wrap', color: SLATE, marginTop: 4, maxHeight: 200, overflowY: 'auto', background: BG, padding: 10, border: `1px solid ${LINE}`, borderRadius: 8 }}>{cs.transcricao}</div></details>}
                </div>
              )}
            </div>
          ))}
      ...
      {tab === 'tarefas' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {KANBAN.map(([st, label, color]) => {
            ...
            return (
              <div key={st} style={{ border: `1px solid ${LINE}`, borderRadius: 12, background: CARD, boxShadow: '0 1px 2px rgba(0,0,0,0.04)', minHeight: 120, overflow: 'hidden' }}>
                <div style={{ background: BG, color, padding: '8px 12px', fontFamily: mono, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', borderBottom: `1px solid ${LINE}` }}>{label} ({items.length})</div>
                <div style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {items.map(a => {
                    const overdue = a.status !== 'DONE' && a.dueDate && new Date(a.dueDate).getTime() < Date.now()
                    const idx = order.indexOf(st)
                    return (
                      <div key={a.id} style={{ background: CARD, border: `1px solid ${LINE}`, borderRadius: 8, padding: 10, fontFamily: mono, fontSize: 12 }}>
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          {idx > 0 && <button onClick={() => moveAction(a.id, order[idx - 1])} style={{ border: `1px solid ${LINE}`, background: CARD, color: SLATE, cursor: 'pointer', fontFamily: mono, fontSize: 11, padding: '3px 8px', borderRadius: 6 }}>←</button>}
                          {idx < 2 && <button onClick={() => moveAction(a.id, order[idx + 1])} style={{ border: `1px solid ${LINE}`, background: CARD, color: SLATE, cursor: 'pointer', fontFamily: mono, fontSize: 11, padding: '3px 8px', borderRadius: 6, marginLeft: 'auto' }}>→</button>}
                        </div>
                      </div>
                    )
                  })}
                  {items.length === 0 && <div style={{ fontFamily: mono, fontSize: 11, color: DIM, textAlign: 'center', padding: 8 }}>—</div>}
                </div>
              </div>
            )
          })}
...
function SessionForm({ clientId, onClose, onSaved }: { clientId: string; onClose: () => void; onSaved: () => void }) {
  ...
  const lbl: React.CSSProperties = { fontFamily: mono, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', color: MUT, display: 'block', marginBottom: 4 }
  const sec: React.CSSProperties = { fontFamily: mono, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: INK, margin: '18px 0 8px', borderBottom: `1px solid ${LINE}`, paddingBottom: 5 }
  ...
  const xBtn: React.CSSProperties = { border: `1px solid ${LINE}`, background: CARD, color: MUT, cursor: 'pointer', padding: '0 10px', borderRadius: 6 }
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose() }} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 16, overflowY: 'auto' }}>
      <div style={{ background: CARD, borderRadius: 12, width: '100%', maxWidth: 560, marginTop: 20, overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
        <div style={{ background: BG, color: INK, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${LINE}` }}>
          <span style={{ fontFamily: disp, fontSize: 15, fontWeight: 700 }}>Registrar sessão / finalizar mentoria</span>
        ...
        <div style={{ padding: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 18 }}>
            <button onClick={onClose} style={{ flex: 1, border: `1px solid ${LINE}`, background: CARD, color: INK, borderRadius: 6, padding: 11, fontFamily: mono, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Cancelar</button>
            <button onClick={save} disabled={saving} style={{ flex: 1, border: 0, background: NEON, color: INK, borderRadius: 6, padding: 11, fontFamily: mono, fontSize: 13, fontWeight: 700, cursor: saving ? 'wait' : 'pointer' }}>{saving ? 'Salvando...' : 'Registrar sessão'}</button>
          </div>
        </div>
      </div>
    </div>
