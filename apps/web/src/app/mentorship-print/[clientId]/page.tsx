...
interface Detail {
  profile: { mentorName: string | null; status: string }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 32, fontFamily: 'Inter, sans-serif', color: '#111', background: 'white' }}>
      ...
      {d.caseStudies[0] && (
        <section style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: 14, textTransform: 'uppercase', color: '#888', borderBottom: '1px solid #ddd', paddingBottom: 4 }}>Últimas métricas ({dt(d.caseStudies[0].sessionDate)})</h2>
      ...
      <section>
        ...
        {d.caseStudies.map(s => (
          <div key={s.id} style={{ borderLeft: '3px solid #111', paddingLeft: 12, margin: '12px 0' }}>
            ...
            {s.situacaoAtual && <p style={{ fontSize: 12, margin: '4px 0' }}><b>Situação:</b> {s.situacaoAtual}</p>}
            {s.oQueTrabalhou && <p style={{ fontSize: 12, margin: '4px 0' }}><b>Trabalhado:</b> {s.oQueTrabalhou}</p>}
