...
export class PersonAccountsService {
  ...
  async findAll() {
    ...
    return persons.map(p => {
      ...
      // Build list of pending debits (not yet fully covered by credits)
      const creditSourceIds = new Set(credits.map(c => c.description))
      const pendingDebits = debits
        .filter(d => new Date(d.date) >= new Date())
        .map(d => ({ date: d.date, value: Number(d.value), description: d.description }))
